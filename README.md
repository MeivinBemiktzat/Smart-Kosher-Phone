...
**ניהול חכם לפלאפון כשר** — תוכנה יחודית שמתחברת לפלאפון כשר דרך הבלוטוס.
המחשב מתחזה לדיבורית בלוטוס, וכך הוא יכול להתחבר לכל פלאפון — גם פלאפון
כשר. בתוכנה הרבה פונקציות, ביניהן: ממשק שיחה, תא קולי, הקלטת שיחות,
ממשק התכתבות, בייביסיטר, ועוד חלקים רבים בפיתוח.

**מצב פיתוח:** נמוך

**תכונות עתידיות לפיתוח:**
- ייצוב התוכנה
- הוספת ופיתוח הפונקציות השונות

---

## הורדה — בניית EXE אוטומטית (GitHub Actions)

בכל דחיפת tag בפורמט `v*` (למשל `v1.0`), או הפעלה ידנית של ה-workflow
תחת לשונית **Actions** בגיטהאב, נבנה אוטומטית קובץ `SmartKosherPhone.exe`
ומועלה הן כ-Artifact של ה-workflow והן ל-**Releases** של המאגר.
...
לאחר סיום הריצה (כמה דקות), הקובץ הבנוי יופיע תחת לשונית
**Releases** של המאגר, מוכן להורדה.
